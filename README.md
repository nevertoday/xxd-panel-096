<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 096 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 096 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体与叙事关系 → 提取一张彩纸色 → 用单一墨色和细钢笔线重构关键结构 → 以老式打字机字混排标题、数字与通信信息 → 加入纸纤维、复印颗粒、磨损与轻微套印偏差
```

## 成品中最容易识别的风格特点

- 下半部分坚持 one coloured paper + one printing ink，色彩张力来自纸色与单一墨色的反差。
- 细钢笔线、断续轮廓、少量排线与不完全均匀的复制颗粒共同塑形，不追求写实阴影。
- 文字是画面组成部分：窄小、机械、略磨损、轻微错位，像直接打在卡片上。
- 背景主动删减，保留少量符号、示意线和技术标记，并让不规则留白参与版面节奏。
- 拒绝商业复古模板、渐变、3D、光滑矢量、彩色插画和无意义伪文字。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-096-prompt.en.md)
...
| **[xxd-panel-096](https://github.com/nevertoday/xxd-panel-096)** | QSL 民间通信卡 · 打字机字 · 彩纸单色复制 · 钢笔线描 · DIY 颗粒 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–096） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
