<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 096 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 096이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

상업적 복고 템플릿은 개인 인쇄의 우연성을 지웁니다. 096은 사진을 촉감 있는 QSL 카드 에페메라로 재구성합니다.

**Panel 096**은 가는 펜선과 제한된 해칭으로 정체성을 남기고, 좁고 작고 닳고 어긋난 타자기 문자를 자유롭게 섞습니다. 선명한 한 가지 종이색과 대비되는 한 가지 인쇄 잉크가 직접적이고 손으로 만든 자신감 있는 화면을 만듭니다.

### 이런 경우에 적합합니다

- QSL 카드, 무선 확인 카드, 개인 알림, 언더그라운드 소책자의 민간 인쇄 감각이 필요할 때
- 선화, 기술 표식, 타이핑 정보, 불규칙한 여백을 자유롭지만 질서 있게 섞고 싶을 때
- 복사 입자, 잉크 농도 차이, 종이 섬유, 글자 마모, 가벼운 맞춤 오차를 활용할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 096로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** QSL 통신 카드 · 민간 에페메라 · 타자기 · 펜 선화 · 색지＋단색 잉크 · DIY 복사 입자
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 개인이 타이핑하고 복사하고 만지고 우편으로 보낸 듯한 통신 카드 |
| **대표 특징** | 한 가지 종이색, 한 가지 잉크, 가는 펜선, 닳은 타자기 글자, DIY 복사 입자 |
...
```text
주체·자세·서사 관계 분석 → 사진에 근거한 한 가지 종이색 선택 → 한 가지 잉크와 가는 펜선으로 핵심 구조 재구성 → 의미 있는 타자기 제목·숫자·통신 정보 혼합 → 복사 입자·글자 마모·종이 섬유·가벼운 맞춤 오차 유지
```

## 완성작의 특징

- 한 가지 색지＋한 가지 인쇄 잉크 체계를 지키며 풍부한 팔레트로 확장하지 않습니다.
- 가늘고 끊긴 펜 윤곽과 소량의 해칭으로 형태를 만들고 사실적 명암을 피합니다.
- 작고 기계적이며 닳은 타자기 글자를 카드 자체에 통합합니다.
- 불규칙한 여백, 어긋난 정렬, 좁은 마진, 국소 밀도로 지면 리듬을 만듭니다.
- 상업적 복고, 무균 그리드, 그라데이션, 3D, 매끈한 벡터, 가짜 문자를 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-096-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 096의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-096 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-096 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-096 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-096 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-096 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-096 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 096 적용, 공통 설정은 한 번만 확정
/xxd-panel-096 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-096`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-096-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-096-prompt.zh-CN.md)
...
| **[xxd-panel-096](https://github.com/nevertoday/xxd-panel-096)** | QSL 통신 카드 · 타자기 · 색지＋단색 잉크 · 펜 선화 · DIY 복사 입자 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–096) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
