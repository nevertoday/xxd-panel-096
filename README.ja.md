<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 096 プロジェクトバナー" width="1200"></p>
...
商業的なレトロテンプレートは私家印刷の偶然性を消します。096 は写真を、触感のある QSL カードのエフェメラへ再構成します。

**Panel 096** は細いペン線と限定的な排線で主体を保ち、細く小さく摩耗してずれたタイプライター文字を自由に混植します。鮮明な一色の紙と対照的な一色の印刷インクが、直接的で手作りの自信ある画面を作ります。

### 向いている用途

- QSLカード、無線確認カード、私製通知、地下小冊子の民間印刷感が欲しい場合
- 線画、技術記号、タイプ情報、不規則な余白を自由かつ秩序立てて混植したい場合
- コピー粒子、墨むら、紙繊維、文字摩耗、軽い版ずれを活かしたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 096 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** QSL通信カード · 民間エフェメラ · タイプライター · ペン線画 · 色紙＋単色墨 · DIYコピー粒子
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 個人がタイプし、複写し、手で扱い、郵送したような通信カード |
| **識別しやすい特徴** | 一色の紙、一色の墨、細いペン線、摩耗したタイプ文字、DIY複写粒子 |
...
```text
主体・姿勢・物語関係を分析 → 写真に根ざす一色の紙色を選ぶ → 一色の墨と細いペン線で重要構造を描く → 意味あるタイプライター題名・数字・通信情報を混植 → コピー粒子・文字摩耗・紙繊維・軽い版ずれを残す
```

## 完成作の特徴

- 一色の色紙＋一色の印刷墨を守り、多色パレットへ広げない。
- 細い途切れた輪郭と少量の排線で形を作り、写実陰影を避ける。
- 小さく機械的で摩耗したタイプ文字をカード本体へ統合する。
- 不規則な余白、ずれた整列、狭いマージン、局所密度で版面を作る。
- 商業レトロ、無菌グリッド、グラデーション、3D、滑らかなベクター、偽文字を避ける。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-096-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 096 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-096 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-096 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-096 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-096 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 096 を適用し、共通設定は一度だけ解決
/xxd-panel-096 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-096`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-096-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-096-prompt.zh-CN.md)
...
| **[xxd-panel-096](https://github.com/nevertoday/xxd-panel-096)** | QSL通信カード · タイプライター · 色紙＋単色墨 · ペン線画 · DIY複写粒子 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–096） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
